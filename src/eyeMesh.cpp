...
void eyeMesh::draw(GLenum drawMode)
{
	...
	for (int i=0; i<(meshGridX-1) ; i++)
    {
		...
		for (int j = 0; j < meshGridY ; j++)
		{
            glTexCoord2f(meshTexCoord[i][j][0], meshTexCoord[i][j][1]);
            glVertex3f( meshVerCoord[i][j][0] ,  meshVerCoord[i][j][1], 0.0);
            glTexCoord2f(meshTexCoord[i+1][j][0], meshTexCoord[i+1][j][1]);
            glVertex3f( meshVerCoord[i+1][j][0] ,  meshVerCoord[i+1][j][1], 0.0);
		}
		glEnd();
	}
